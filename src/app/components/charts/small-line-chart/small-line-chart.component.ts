        ...
        ...this.options, tooltips: {
          ...
          custom(tooltipModel) {
            if (tooltipModel && tooltipModel.dataPoints) {
              const yLabel:any = tooltipModel.dataPoints[0].yLabel;
              const xLabel:any  = tooltipModel.dataPoints[0].xLabel;
              const label:any  = tooltipModel.body[0].lines[0].split(':')[0];
      ...
      plugins: [
        {
          afterInit(chart, options) {
            const yLabel:any  = chart.data.datasets[0].data[0];
            const xLabel:any  = chart.data.labels[0];
            const label:any  = chart.data.datasets[0].label;
            thisRef.changeState(yLabel, label + '-' + xLabel);
          }
        }
      ]
