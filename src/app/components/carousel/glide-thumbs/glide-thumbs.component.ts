...
export class GlideThumbsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.glideCarouselImages = new Glide(this.glideRef.nativeElement, { ...this.settingsImages, direction: this.translate.instant('settings.direction') });
    this.glideCarouselThumbs = new Glide(this.glideThumbsRef.nativeElement, { ...this.settingsThumbs, direction: this.translate.instant('settings.direction') });
