...
export interface IcConsumerBlocked {
  id: number;
  username: string;
  name: string;
  email: string;
...
export interface IcConsumerBlockedResponse {
  data: IcConsumerBlocked[];
  status: boolean;
  totalItem: number;
  totalPage: number;
  pageSize: string;
...
export class ApiServiceCliConsumerBlocked {
  ...
  getConsumerBlocked(pageSize: number = 10, currentPage: number = 1, search: string = '', orderBy: string = '') {
    ...
    return this.http.get(url, { headers, params})
      .pipe(
        map((res: IcConsumerBlockedResponse) => {
          return res;
        }),
        catchError(errorRes => {
